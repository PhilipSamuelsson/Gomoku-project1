...
const restartGame = async () => {
    try {
        const response = await fetch('https://gomoku-server-e8c8db855df9.herokuapp.com/api/reset-game', {
            method: 'POST',
        });
        if (response.ok) {
    ...
    useEffect(() => {
        ...
        const fetchInitialGameBoard = async () => {
            try {
                const response = await fetch('https://gomoku-server-e8c8db855df9.herokuapp.com/api/get-board');
                if (response.ok) {
...
function checkForWin(board, row, col, player) {
    ...
    const sendMoveToServer = async (row, col, player) => {
        try {
            const response = await fetch('https://gomoku-server-e8c8db855df9.herokuapp.com/api/make-move', {
                method: 'POST',
                headers: {
